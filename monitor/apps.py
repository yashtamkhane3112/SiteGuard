...
def log_session_startup_diagnostics():
    ...
    logger.info(
        "Session startup diagnostics: engine=%s cookie_age=%s secure=%s httponly=%s samesite=%s save_every_request=%s expire_at_browser_close=%s proxy_ssl_header=%s use_x_forwarded_host=%s csrf_secure=%s csrf_httponly=%s csrf_samesite=%s secret_key_present=%s backend_health=%s",
        session_engine,
        int(getattr(settings, "SESSION_COOKIE_AGE", 0) or 0),
        bool(getattr(settings, "SESSION_COOKIE_SECURE", False)),
        bool(getattr(settings, "SESSION_COOKIE_HTTPONLY", False)),
        ...
        bool(getattr(settings, "CSRF_COOKIE_HTTPONLY", False)),
        ...
        extra={
            ...
            "secret_key_present": bool(getattr(settings, "SECRET_KEY", "")),
