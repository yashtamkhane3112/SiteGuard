...
from monitor.emailing import (
    build_password_reset_preview_url,
    get_email_base_url,
    ...
    def handle(self, *args, **options):
        ...
        if kind == 'password_reset':
            ...
            if reset_user is None:
                reset_user = User.objects.create(
                    username=f'resend-reset-{timezone.now().strftime("%Y%m%d%H%M%S")}',
                    email=recipient,
                )
            ...
            if form._last_send_succeeded:
                ...
                self.stdout.write(self.style.ERROR('Password reset email failed. Check provider settings and logs.'))
            return
        ...
        if kind == 'operational':
            ...
            html_body = (
                ...
                '<p>This verifies transactional delivery, HTML rendering, and sender identity.</p>'
        ...
        if sent:
            ...
            self.stdout.write(self.style.ERROR('Email failed. Check provider settings and logs.'))
